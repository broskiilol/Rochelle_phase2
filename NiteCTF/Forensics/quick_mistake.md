...
## Solution:
- I open the .pcap file and start looking around. I first see the tcp stream and find an interesting one of telemetry. In the frames after this, there are various handshakes.
- I open one of the handshakes (the longest one) and i can see that it is something related to the sslkeylog.
  <img width="1910" height="1000" alt="image" src="https://github.com/user-attachments/assets/30d15c12-b8fe-42ca-afb5-b983278f8071" />

- The SCID of this is `2457ce19cb87e0eb` and the attacker IP address is `192.0.2.66` thus we found two parts of the flag
- For the next part, it says flag part 2, so i look at the contents of the handshake. There are two chunks (so flag part 2 must mean the second chunk). They have given a `nonce`, `seed`, `ct`, and `tag`. This suggests something to do with AES-GCM algorithm
- - Key of this is `"seed": "af717e2c8789db71fe624598faba3953c23fdb685e6b8cd2e6f84beef0c57175"`
  - For first chunk the information is
...
if __name__ == "__main__":
    main()
```
Using this i decode the information, thus getting the following
```
...
```
- Using `Wireshark -> Preferences -> Protocols -> TLS` i decrypt HTTP/3 traffic and filter for http3. The data has been sent in tar.gz form so we extract that in .env getting AES secret key
  `AES_FLAG_KEY=wEN64tLF1PtOglz3Oorl7su8_GQzmlU2jbFP70cFz7c=`
- Using this i can decrypt for the flag.
```py
